...
//     [topCell addSubview:lotteryView];
    ...
    UIImageView *lottteryLogo = [[UIImageView alloc]initWithFrame:CGRectMake(10 ,12, 68.5, 68.5)];
    lottteryLogo.userInteractionEnabled = NO;
    ...
    //写死，不要问。
    if([object.cnname isEqualToString:@"河内Quick5"])
    {
    ...
    if ([kindlottery.cnname isEqualToString:@"河内Quick5"]) {
    ...
    [lottteryLogo setImage:backgroundImage];

    [bgView addSubview:lottteryLogo];
}
...
- (void)updateTime:(NSNotification *)notification
{
    ...
    } else if ([kindOfLottery.cnname isEqualToString:@"河内Quick5"]) {
