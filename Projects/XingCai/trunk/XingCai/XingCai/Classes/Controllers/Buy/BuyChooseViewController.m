...
- (void)initSubviews
{
    ...
    timerlabel = [[UILabel alloc] initWithFrame:CGRectMake(0, timerlabelHeight, 200, 17)];
    timerlabel.font = [UIFont systemFontOfSize:10];
    timerlabel.textColor = [UIColor colorWithRed:(187/255.0) green:(163/255.0) blue:(136/255.0) alpha:1];
    timerlabel.textAlignment = UITextAlignmentRight;
    timerlabel.backgroundColor = [UIColor clearColor];
    timerlabel.text = @"距离截止时间：";
    [self.view addSubview:timerlabel];
    
    timerHMS= [[UILabel alloc] initWithFrame:CGRectMake(201, timerHMSHeight, 160, 17)];
    timerHMS.font = [UIFont systemFontOfSize:10];
    ...
    for (int i = 0; i < BallButtonPoolSize; ++i) {
        ...
        [ball setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
        [ball setBackgroundImage:[UIImage imageNamed:@"ball_whitenormal"] forState:UIControlStateNormal];
        [ball setBackgroundImage:[UIImage imageNamed:@"ball_redclick"] forState:UIControlStateSelected];
    ...
    //第一级菜单
    firstMenuBoard = [[UIView alloc] initWithFrame:CGRectMake(10, 132, 299, 244)];
    firstMenuBoard.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"sec_menu_bg"]];
    ...
    for (int i = 0; i < ruleAndMenuArray.count; ++i)
    {
        ...
        [secMenuBtn setTitleColor:[UIColor colorWithRed:(19/255.0) green:(126/255.0) blue:(246/255.0) alpha:1] forState:UIControlStateNormal];
        [secMenuBtn setTitleColor:[UIColor colorWithRed:(255/255.0) green:(255/255.0) blue:(255/255.0) alpha:1] forState:UIControlStateSelected];
        ...
        if (i % 2 == 0)
        {
            secMenuBtn.frame = CGRectMake(35, 15+22*i, 97, 32);
        }
        else
        {
            secMenuBtn.frame = CGRectMake(166, 15+22*(i-1), 97, 32);
        }
...
- (void)tryToGetJiangqi
{
    ...
    timerlabel.text = [NSString stringWithFormat:@"距离%@期截止时间：",[[JiangQiManager sharedManager] getCurrentJiangQiWithLotteryId:second.lotteryid]];
...
{
    timerlabel.text = [NSString stringWithFormat:@"距离%@期截止时间：",notification.object];
}
#pragma mark - 菜单和玩法
- (void)getMenuAndRule
{
...
//顶部标题按钮事件
- (IBAction)selectGame:(id)senderx
{
...
-(void)updateHorizontalMenuAtIndex:(NSInteger)index
{
    ...
    for (int k=0; k<verticalMenuItems.count; k++)
    {
        ...
        {
            hdBtn.frame = CGRectMake(110, 0, 97, 53.5);
        }
        else if (verticalMenuItems.count == 2)
        {
            hdBtn.frame = CGRectMake(60+105*k, 0, 97, 53.5);
        }
        else
        {
            hdBtn.frame = CGRectMake(6+105*k, 0, 97, 53.5);
        }
...
//    NSLog(@"%@--",fouMo.name);
    ...
    if (isInputType) {
        tipsLable.frame=CGRectMake(20, 5, 280, 28.5);
         tipsLable.font = [UIFont systemFontOfSize:11];
    }else{
        tipsLable.frame=CGRectMake(155, 5, 155, 28.5);
    ...
    tipsLable.textColor = [UIColor colorWithRed:(187/255.0) green:(163/255.0) blue:(136/255.0) alpha:1];
...
//    lotteryName=fouMo.name;
    ...
    if (layout && [layout respondsToSelector:@selector(count)] && layout.count > 0)
    {
        ...
        for (int i = 0; i < layout.count; i++)
        {
            ...
            //计算背景高度
            unsigned long selectAreaHeigth = ((ballArray.count - 1) / 5 + 1) * 48;
            totalHeight = (selectAreaHeigth + 10)*place + 8;
            //灰色背景view
            UIView *bgview = [[UIView alloc]initWithFrame:CGRectMake(0, totalHeight+25, 320, selectAreaHeigth)];
            ...
            if (![title isEqualToString:@""])
            {
                ...
                if (title.length < 4)
                {
                    weiFlagBg = [[UIImageView alloc] initWithFrame:CGRectMake(1, 5, 35, 18)];
                    [weiFlagBg setImage:[UIImage imageNamed:@"bk_tag"]];
                    ballTitle = [[UILabel alloc] initWithFrame:CGRectMake(1, 5, 30, 18)];
                }
                else
                {
                    weiFlagBg = [[UIImageView alloc] initWithFrame:CGRectMake(1, 5, 45, 18)];
                    [weiFlagBg setImage:[UIImage imageNamed:@"bk_longtag"]];
                    ballTitle = [[UILabel alloc] initWithFrame:CGRectMake(1, 5, 40, 18)];
                }
                [ballTitle setBackgroundColor:[UIColor clearColor]];
                ballTitle.font = [UIFont systemFontOfSize:10];
                ballTitle.textColor = [UIColor colorWithRed:(202/255.0) green:(143/255.0) blue:(17/255.0) alpha:1];
            ...
            for (int i = 0; i < ballArray.count; i++)
            {
                ...
                ball.frame = CGRectMake(62 + 50 * col, 46 * row, 42, 42);
        ...
        self.ballScrollView.contentSize =CGSizeMake(320, moneyModesHight);
    }
    else
    {
...
- (LotteryInformation *)getLotteryInformationWithCodes:(NSString *)codes nums:(NSString *)nums
{
    ...
    bettingInformation.omodel = [NSString stringWithFormat:@"%ld",[UserInfomation sharedInfomation].prizeType];
