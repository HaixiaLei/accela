...
- (BOOL)userInputCorrect
{
    ...
    NSString *amount_minStr = self.withdrawObject.min_money;
    NSString *amount_maxStr = self.withdrawObject.max_money;
    ...
    [[AppHttpManager sharedManager] queRenTiKuangXinXiWithCheck:self.check bankInfo:bankInfo money:self.moneyAmountTF.text Block:^(id JSON, NSError *error)
     {
         if (!error)
         {
             if ([JSON isKindOfClass:[NSDictionary class]])
             {
                 ...
                 //提交
                 [[AppHttpManager sharedManager] zuiZhongTiKuangXinXiWithCheck:self.check cardId:cardid money:money Block:^(id JSON, NSError *errcor)
                  {
