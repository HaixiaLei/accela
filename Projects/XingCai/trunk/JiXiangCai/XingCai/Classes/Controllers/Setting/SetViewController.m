...
- (IBAction)feedback:(id)sender
{
    ...
    // fix当iphone4/4s 键盘会遮挡输入框文字（只有使用ios8带智能识别键盘或使用第三方键盘的时候会遮挡）
    float toFitIphone4s = 0;
    float version = [[[UIDevice currentDevice] systemVersion] floatValue];
    if (IS_IPHONE4 && version >=8 ) {
        toFitIphone4s = -37;
    }
   ...
   self.feedbackView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, 320, 199+toFitIphone4s)];
    ...
    self.feedbackTextview = [[UITextView alloc]initWithFrame:CGRectMake(0, 50, 320, 130+toFitIphone4s)];
    ...
    UILabel * tiplabel = [[UILabel alloc]initWithFrame:CGRectMake(10, 173+toFitIphone4s, 100, 30)];
    tiplabel.text = @"还能输入字数:";
    tiplabel.font = [UIFont systemFontOfSize:15];
    tiplabel.textColor =[UIColor grayColor];
    [self.feedbackView addSubview:tiplabel];
    
    self.leftLabel = [[UILabel alloc]initWithFrame:CGRectMake(110, 173+toFitIphone4s, 30, 30)];
    self.leftLabel.text = @"250";
...
-(BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
    ...
    if (textView.text.length >= 250 && string.length >= 250) {
        UIAlertView * alert=[[UIAlertView alloc] initWithTitle:@"提示" message:@"您已输入250个字" delegate:nil cancelButtonTitle:@"返回" otherButtonTitles: nil];
        [alert show];
    ...
    if (string.length > 250) {
        UIAlertView * alert=[[UIAlertView alloc] initWithTitle:@"提示" message:@"输入不能超过250个字" delegate:nil cancelButtonTitle:@"返回" otherButtonTitles: nil];
        [alert show];
        
        return NO;
    }else
    {
        self.leftLabel.text = [NSString stringWithFormat:@"%lu",250-string.length];
        return YES;
    }
