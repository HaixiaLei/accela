...
//    [topView setBackgroundColor:[UIColor colorWithPatternImage:[UIImage imageNamed:@"tag_content_num_bk"]]];
...
- (IBAction)confirmBet:(id)sender
{
    ...
    JiangQiObject *jiangQiObject = [[JiangQiManager sharedManager] getJiangQiObjectWithLotteryId:self.lotteryid];
    ...
    {
        NSString *currentJiangQi = [[JiangQiManager sharedManager] getCurrentJiangQiWithLotteryId:self.lotteryid];
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    NSArray *keZhuiHaoJiangQis = [[JiangQiManager sharedManager] getKeZhuiHaoJiangQisWithLotteryId:self.lotteryid];
...
- (void)updateBettingInformations
{
    ...
    if (totalWidth < 160) {
        int origin = screenWidth * 0.5 - totalWidth * 0.5-25;
    ...
    {
        self.betNumber.point = CGPointMake(screenWidth * 0.4 - self.betNumber.frame.size.width * 0.5, 5);
        self.betMoney.point = CGPointMake(screenWidth * 0.4- self.betMoney.frame.size.width * 0.5 - self.yuanLabel.frame.size.width * 0.5, 5 + self.betNumber.frame.size.height);
...
//    static NSString *testCell=@"BetNumberCell";
    NSInteger numberOfRows = [self tableView:tableView numberOfRowsInSection:0];
    if (numberOfRows == 1 || (numberOfRows > 1 && indexPath.row == numberOfRows - 1))
    {
        UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:betNumberBottomCell];
        if (!cell) {
            cell=[[UITableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:betNumberBottomCell];
    ...
    {
        UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:betNumberCell];
        if (!cell) {
            cell=[[NSBundle mainBundle] loadNibNamed:@"BetNumber_Cell" owner:self options:nil].lastObject;
           
        }
//        [cell setBackgroundView:[[UIImageView alloc]initWithImage:[UIImage imageNamed:@"tag_content_num_bk"]]];
//        [cell setSelectedBackgroundView:[[UIImageView alloc]initWithImage:[UIImage imageNamed:@"tag_record_content_h"]]];
        UILabel *betNumberLabel=(UILabel *)[cell.contentView viewWithTag:103];
        UILabel *descLabel=(UILabel *)[cell.contentView viewWithTag:104];
        UILabel *numberOfBetLabel=(UILabel *)[cell.contentView viewWithTag:105];
        UILabel *priceLabel=(UILabel *)[cell.contentView viewWithTag:106];
        UIButton *deleteButton=(UIButton *)[cell.contentView viewWithTag:102];
        
        LotteryInformation *lotteryInformation = [lotteryInformations objectAtIndex:indexPath.row];
        betNumberLabel.text = lotteryInformation.bettingInformation.showCodes;
        descLabel.text = lotteryInformation.bettingInformation.showDesc;
        numberOfBetLabel.text = [NSString stringWithFormat:@"%@注",lotteryInformation.bettingInformation.nums];
        priceLabel.text = [NSString stringWithFormat:@"%@元",[Utility formatStringFromDouble:(double)([lotteryInformation.bettingInformation.money longLongValue] / [lotteryInformation.bettingInformation.times integerValue]) / RateToInt]];
...
{
    NSString *currentJiangQi = notification.object;
