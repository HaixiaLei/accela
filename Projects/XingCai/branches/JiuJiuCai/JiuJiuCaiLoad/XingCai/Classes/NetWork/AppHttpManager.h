...
/*
 参数说明：
 newloginpass : md5加密后的密码（使用类似js的$.md5()加密）
 securitypass : md5加密后的密码（使用类似js的$.md5()加密）
 userid	:  用户名
...
/*
 参数说明：
 username —— 用户名
 loginpass —— md5加密后的密码（使用类似js的$.md5()加密）
 
 返回：
 {
 "sess":"a1e98a0c69f118e2b8299fa4e14f4cddbf70b8f2"
 }
 
 返回说明：
 sess —— 登录成功后的session id，请附加在以下所有接口后。例：
 feed/index.php?controller=game&action=issueindex&sess=8d08420813f4e058c26cf1caa556b9670
