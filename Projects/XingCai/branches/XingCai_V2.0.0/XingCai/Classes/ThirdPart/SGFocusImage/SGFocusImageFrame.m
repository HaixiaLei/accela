...
- (void)setupViews
{
    ...
    _pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(self.bounds.size.width *.7, self.bounds.size.height - size.height+10, size.width, size.height)];
...
-(void)clickPageImage:(UIButton *)sender{
    DDLogInfo(@"click button tag is %ld",sender.tag);
}
