...
{
    [super viewDidLoad];
    [self setupNavigationBarTitle:@"更多" tintColor:GUI_COLOR_NAVIGATION_BAR_TEXT navigationBarHidden:NO navigationBarTranslucent:NO];
    self.bgscrollview.contentSize = CGSizeMake(320, 500+(IS_IPHONE5?5:4));
   
}
-(void)getNewMessages
{
    [[AppHttpManager sharedManager] getUnreadMessages:^(id JSON, NSError *error)
     {
         if (!error)
         {
             if ([JSON isKindOfClass:[NSDictionary class]])
             {
                 ...
                 if (count.integerValue != 0)
                 {
                     if (!bubble)
                     {
                         bubble = [[BubbleView alloc] initWithFrame:CGRectMake(self.btnMyMessage.frame.size.width*3.3/5, self.btnMyMessage.frame.size.height*2/11, 50, 22)];
                         [self.btnMyMessage addSubview:bubble];
                     }
                     bubble.hidden = NO;
                     [bubble setText:count];
                 }
                 else if (bubble)
                 {
                     bubble.hidden = YES;
