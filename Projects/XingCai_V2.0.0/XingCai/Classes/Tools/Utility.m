...
+ (BOOL)containUIAlertView:(UIView *)view andSameMessage:(NSString *)message
{
    BOOL findOne = NO;
    NSArray* subviews = view.subviews;
    if (subviews && subviews.count > 0)
    {
        for (UIView *viewInSubview in subviews)
        {
            if (SystemVersion >= 7.0)
            {
                ...
                if (topMostAlert)
                {
                    if ([topMostAlert.message isEqualToString:message])
                    {
                        findOne = YES;
                        return findOne;
                    }
                ...
                {
                    findOne = [Utility containUIAlertView:viewInSubview andSameMessage:message];
                }
            }
            else
            {
                if ([viewInSubview isKindOfClass:[UIAlertView class]])
                {
                    UIAlertView *alertView = (UIAlertView *)viewInSubview;
...
+ (UIAlertView *)showErrorWithMessage:(NSString *)message delegate:(id)delegate
{
    return [Utility showErrorWithMessage:message delegate:delegate tag:0];
}

+ (UIAlertView *)showErrorWithMessage:(NSString *)message delegate:(id)delegate tag:(NSInteger)tag
{
    return [Utility showErrorWithMessage:message delegate:delegate tag:tag duplicationPrevent:YES];
}

+ (UIAlertView *)showErrorWithMessage:(NSString *)message delegate:(id)delegate tag:(NSInteger)tag duplicationPrevent:(BOOL)duplicationPrevent
{
    return [Utility showErrorWithMessage:message delegate:delegate cancelButtonTitle:nil otherButtonTitles:@"确定" tag:tag duplicationPrevent:duplicationPrevent];
}

+ (UIAlertView *)showErrorWithMessage:(NSString *)message delegate:(id)delegate cancelButtonTitle:(NSString *)cancelTitle otherButtonTitles:(NSString *)otherTitle tag:(NSInteger)tag duplicationPrevent:(BOOL)duplicationPrevent
{
...
+ (UIAlertView *)showErrorWithTittle:(NSString *)title message:(NSString *)message delegate:(id)delegate cancelButtonTitle:(NSString *)cancelTitle otherButtonTitles:(NSString *)otherTitle tag:(NSInteger)tag duplicationPrevent:(BOOL)duplicationPrevent
{
...
//            {
//                if ([Utility containUIAlertView:window andSameMessage:message])
//                {
//                    shouldShow = NO;
//                    break;
//                }
//            }
            NSArray *allAlertViews = [AppAlertView allAlertViews];
            for (AppAlertView *alertView in allAlertViews)
            {
                if ([alertView.message isEqualToString:message])
                {
                    shouldShow = NO;
        ...
        if (shouldShow)
        {
            AppAlertView *alertView = [[AppAlertView alloc] initWithTitle:@"提示" message:message delegate:delegate cancelButtonTitle:cancelTitle otherButtonTitles:otherTitle,nil];
            alertView.tag = tag;
            [alertView show];
