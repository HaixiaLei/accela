...
{
    //如果传入字符串的int值小于1000，则返回
    if (money.integerValue < 1000) {
        return money;
    }
    
    NSMutableString *integerString = nil;
    
    //纪录下小数部分
    NSString *decimalString = @"";
    NSRange decimalRange = [money rangeOfString:@"."];
    if (decimalRange.location != NSNotFound) {
        decimalString = [money substringFromIndex:decimalRange.location];
        integerString = [[NSMutableString alloc]initWithString: [money substringToIndex:decimalRange.location]];
    }else{
        integerString = [[NSMutableString alloc]initWithString:money];
    }
    
    //对整数部分进行加逗号处理
    NSRange dotRange = NSMakeRange(integerString.length, 1000);
    while (dotRange.location > 3) {
        [integerString insertString:@"," atIndex:dotRange.location-3];
        dotRange.location = dotRange.location - 3;
    }
...
//用iOS系统类NSDecimailNumber进行金额乘法运算
