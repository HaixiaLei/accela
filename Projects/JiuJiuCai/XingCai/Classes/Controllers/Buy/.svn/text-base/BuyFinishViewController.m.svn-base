...
#define IOS_VIERSION [[UIDevice currentDevice].systemVersion doubleValue]
...
//    toolBar.frame = toolBarFrame;
    ...
    //设置顶部标题
    if ([self.buyChooseViewController.kindOfLottery.cnname isEqualToString:@"重庆时时彩"]) {
        [self.titleLabel setText:@"重庆时时彩投注列表"];
    }else if ([self.buyChooseViewController.kindOfLottery.cnname isEqualToString:@"河内Quick5"]) {
        [self.titleLabel setText:@"印尼五分彩投注列表"];
    }
}
...
- (void)addKeyboardHandler
{
    ...
    float viewHeight = self.view.frame.size.height;
//    float bottomHeight = self.bottomView.frame.size.height;
    CGRect originToolBarFrame = toolBar.frame;
...
//            }
//            toolBar.frame = toolBarFrame;
            toolBar.frame = originToolBarFrame;
        }
        else
        {
            CGRect toolBarFrame = toolBar.frame;
...
- (IBAction)confirmBet:(id)sender
{
    ...
    if (self.beginChaseSelectButton.selected) {
        NSMutableString *message = [NSMutableString stringWithFormat:@"确定要追号%d期?\n",self.timeOfChase];
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    if (textField == self.chaseTimesTF) {
        if ([newText integerValue] > 120) {
            ...
            self.chaseTimesTF.text = [NSString stringWithFormat:@"%d",keZhuiHaoJiangQis.count];
            return NO;
        }
    }
...
{
    int count = lotteryInformations.count;
...
- (void)deleteLotteryInformationAtIndex:(int)index
{
...
- (void)updateBettingInformations
{
    ...
    {
        self.betNumber.point = CGPointMake(screenWidth * 0.5 - self.betNumber.frame.size.width * 0.5, 5);
        self.betMoney.point = CGPointMake(screenWidth * 0.5 - self.betMoney.frame.size.width * 0.5 - self.yuanLabel.frame.size.width * 0.5, 5 + self.betNumber.frame.size.height);
...
- (void)payMoney
{
    ...
    if (self.beginChaseSelectButton.selected == YES) {
        ...
        NSString *lt_trace_count_input = [NSString stringWithFormat:@"%d",self.timeOfChase];
...
{
    return lotteryInformations.count;
}
