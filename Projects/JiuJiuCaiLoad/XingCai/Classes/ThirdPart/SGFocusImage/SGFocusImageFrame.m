...
- (void)setupViews
{
    ...
    NSLog(@"%f---%f---%f----%f",self.bounds.size.height,self.bounds.size.width,self.bounds.origin.x,self.bounds.origin.y);
    CGSize size = CGSizeMake(100, 44);
    _pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(self.bounds.size.width *.5 - size.width *.5+120, self.bounds.size.height - size.height-170, size.width, size.height)];
    ...
    */
    ...
    _scrollView.contentSize = CGSizeMake(_scrollView.frame.size.width * imageItems.count,300);
...
-(void)clickPageImage:(UIButton *)sender{
    DDLogInfo(@"click button tag is %d",sender.tag);
}
