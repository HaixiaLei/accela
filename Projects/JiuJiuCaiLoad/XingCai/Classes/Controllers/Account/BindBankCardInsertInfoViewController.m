...
- (void)textFieldDidBeginEditing:(UITextField *)textField
{
    ...
    {
        self.view.point = CGPointMake(self.view.frame.origin.x,(IS_IPHONE5?-124:-190));
    }
    else
    {
        self.view.point = CGPointMake(self.view.frame.origin.x,(IS_IPHONE5?-124:-212));
    }
    [UIView commitAnimations];
}
-(BOOL) textFieldShouldReturn:(UITextField *)textField
{
    ...
    {
        if ([self isNameValid:self.accountNameTxt.text])
        {
            [self.accountNoTxt becomeFirstResponder];
        }
        else
        {
            [Utility showErrorWithMessage:@"开户人姓名不符合规则。"];
            return NO;
        }
    }
    return true;
...
//增加正则判断姓名输入是否符合规则
-(BOOL)isNameValid:(NSString *)text{
    NSString *regex = @"^[\u4e00-\u9fa5]+[\u4e00-\u9fa5\.\。\·]*[\u4e00-\u9fa5]+$";
    NSPredicate *predicate = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", regex];
    BOOL isValid = [predicate evaluateWithObject:text];
    isValid = isValid && (text.length <= 16 && text.length >= 2);
    return isValid;
}
